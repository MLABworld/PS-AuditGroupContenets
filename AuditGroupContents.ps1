...
# Hooks into System Center Configuration Manager for the user lookups
#========================================================================================
#
    ...
    {   # Array to hold the computer names and last logon
        ...
        $GroupObj = Get-ADGroupMember $Grp
            ...
            foreach($member in $GroupObj)
            {
                ...
                $userAcct = (Get-WmiObject -namespace root\sms\site_<SITE> -computer <SCCM Primary Site Server> -Query "SELECT SMS_R_System.LastLogonUserName FROM SMS_R_System WHERE Name='$computer'").LastLogonUserName
        ...
        # Lookup ManagedBy Distinguished Name object
         ...
         ##########
             Send-MailMessage `
             -From "<Return Address>@<DOMAIN>.com" `
             -To $MgrEmail `
